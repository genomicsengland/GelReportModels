    ...
    def migrate_clinical_report_rd(self, old_instance):
        ...
        if old_instance.additionalAnalysisPanels is not None:
            new_instance.additionalAnalysisPanels = [
                self.migrate_analysis_panel(old_panel=panel) for panel in old_instance.additionalAnalysisPanels
            ]
    ...
    def merge_annotations_and_frequencies(numeric_annotations, allele_frequencies):
        ...
        if allele_frequencies is not None:
            for af in allele_frequencies:
                if numeric_annotations.get("{}:{}".format(af.study, af.population)) is not None:
                    logging.warning(
                        "{}:{} already exists in numeric_annotations with value {} instead of {}".format(
                            af.study, af.population, numeric_annotations.get("{}:{}".format(af.study, af.population)), af.alternateFrequency
                        )
                    )
                else:
