...
class TestCreateCancerInterpretationRequest(TestCase):
    ...
    def test_create_cancer_interpretation_request(self):
        test_analysis_uri = ''
        test_cancer_participant = get_valid_empty_cancer_participant()
        test_annotation_file = get_valid_empty_file(file_type=FileType.ANN)
        test_bam = get_valid_empty_file(file_type=FileType.BAM)
        test_big_wig = get_valid_empty_file(file_type=FileType.BigWig)
        test_vcf = get_valid_empty_file(file_type=FileType.VCF_somatic_small)
        test_version_control = get_valid_report_version_control()
        test_reported_somatic_variant = get_valid_empty_reported_somatic_variant()
        test_reported_somatic_structural_variant = get_valid_reported_somatic_structural_variant()
